...
public interface DeviationProvider {
  ...
  /**
   * Gets the deviation for current trajectory.
   *
   ...
   * (Don't actually use this formula)
   *
   * While 'Noise' from the {@link (com.github.joonasvali.naturalmouse.api.NoiseProvider)} is generating an offset
   * from the original trajectory and is appended to the original trajectory, having a possibility of accumulating
   * over time.
   *
   * As deviation is a 'function' it should not include Random behaviour, thus Random is not included as a parameter.
   *

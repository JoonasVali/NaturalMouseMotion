  ...
  private double[] normalizeBuckets(double[] flowCharacteristics) {
    ...
    for (int i = 0; i < flowCharacteristics.length; i++) {
      if (flowCharacteristics[i] < 0) {
        throw new IllegalArgumentException("Ind FlowCharacteristics at [" + i + "] : " + flowCharacteristics[i]);
      }
      sum += flowCharacteristics[i];
    }

    if (sum == 0) {
      throw new IllegalArgumentException("Ind FlowCharacteristics. All array elements can't be 0.");
    }
    /*
